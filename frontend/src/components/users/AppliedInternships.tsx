...
const STATUS_CONFIG: Record<AppStatus, { color: string; bg: string; icon: React.ElementType; label: string }> = {
  ...
  "Interview Scheduled": { color: "text-primary",          bg: "bg-primary/10 border-primary/20", icon: Calendar,     label: "Interview" },
...
function DetailDrawer({ intern, onClose }: { intern: AppliedInternship; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end bg-black/40 lg:items-stretch" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-surface px-4 py-3">
          <span className="font-semibold text-sm truncate pr-4">{intern.role}</span>
        ...
        <div className="flex-1 space-y-4 p-4">
          <div className="flex items-start gap-3 rounded-xl border border-border p-4">
            ...
            <div className="min-w-0">
              <p className="font-bold text-sm">{intern.company}</p>
          ...
          <div className="rounded-xl border border-border p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] text-muted-foreground">Updated {formatDate(intern.lastUpdate)}</span>
            </div>
            <ProgressBar status={intern.status} />
          </div>
          ...
          {intern.status === "Selected" && (
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">You've been selected. Check your email for onboarding details.</p>
            </div>
          )}
          ...
          {intern.status === "Rejected" && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4">
              ...
              {intern.rejectionReason && (
                <p className="text-xs text-muted-foreground mt-1">{intern.rejectionReason}</p>
              )}
              <p className="text-[11px] text-muted-foreground mt-2">Keep applying — the right opportunity is around the corner.</p>
            </div>
          )}
        </div>
      </div>
...
function InternshipCard({ intern, onView }: { intern: AppliedInternship; onView: () => void }) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="font-semibold text-sm truncate">{intern.role}</p>
              <p className="text-xs text-muted-foreground truncate">{intern.company}</p>
            </div>
            <StatusBadge status={intern.status} />
          </div>
          ...
          {intern.status === "Interview Scheduled" && intern.interviewDate && (
            <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-primary/5 border border-primary/15 px-3 py-1.5">
              ...
              <p className="text-[11px] text-primary font-medium">Interview: {intern.interviewDate}</p>
            </div>
          )}
          ...
          {intern.status === "Offer Received" && intern.offerStipend && (
            <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-primary/5 border border-primary/15 px-3 py-1.5">
              ...
              <p className="text-[11px] text-primary font-medium">Offer: {intern.offerStipend}</p>
            </div>
          )}
          ...
          {intern.status === "Rejected" && (
            <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-red-50 border border-red-100 px-3 py-1.5">
              ...
              <p className="text-[11px] text-red-600 truncate">
                Not selected{intern.rejectionReason ? ` — ${intern.rejectionReason}` : ""}
              </p>
            </div>
          )}
          ...
          <div className="mt-3 flex items-center justify-between">
            <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
              <Clock className="h-3 w-3" /> Applied {formatDate(intern.appliedDate)}
            </span>
            <button
              onClick={onView}
...
export function AppliedInternships() {
  ...
  const filtered = internships.filter((i) => {
    const matchTab =
      activeTab === "all" ||
      (activeTab === "Under Review" && ["Under Review", "Shortlisted"].includes(i.status)) ||
      i.status === activeTab;
    const q = search.toLowerCase();
    const matchSearch = !q || i.role.toLowerCase().includes(q) || i.company.toLowerCase().includes(q);
    return matchTab && matchSearch;
  });

  const counts = {
    all: internships.length,
    "Under Review": internships.filter((i) => ["Under Review", "Shortlisted"].includes(i.status)).length,
    "Interview Scheduled": internships.filter((i) => i.status === "Interview Scheduled").length,
    "Offer Received": internships.filter((i) => ["Offer Received", "Selected"].includes(i.status)).length,
    Rejected: internships.filter((i) => i.status === "Rejected").length,
  };

  const stats = [
    { label: "Total Applied", value: internships.length },
    { label: "In Progress",   value: internships.filter((i) => !["Rejected", "Selected"].includes(i.status)).length },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        {TABS.map((tab) => {
          const count = counts[tab.id as keyof typeof counts] ?? internships.length;
          return (
            <button
              key={tab.id}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="font-semibold text-sm">No internships found</p>
          <p className="text-xs text-muted-foreground mt-1">
            {search ? "Try a different search term." : "You haven't applied to any internships yet."}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((intern) => (
            <InternshipCard key={intern.id} intern={intern} onView={() => setSelected(intern)} />
          ))}
        </div>
      )}

      {selected && <DetailDrawer intern={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}